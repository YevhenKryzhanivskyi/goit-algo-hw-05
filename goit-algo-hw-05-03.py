...
def read_text(path):
    """Read text from file with robust encoding fallbacks."""
    # Try plain UTF-8
    try:
...
def boyer_moore(text, pattern):
    """Алгоритм Боєра-Мура (спрощений варіант)."""
    n, m = len(text), len(pattern)
    if m == 0 or m > n:
        return -1
...
if __name__ == "__main__":
    # Robust loading of texts
    text1 = read_text("стаття1.txt")
    text2 = read_text("стаття2.txt")
